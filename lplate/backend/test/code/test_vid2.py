...
# Video path
# video_path = "./demo.mp4"
# video_path = "http://192.168.29.201:8080/browserfs.html"
video_path = 0
...
# Define the regex pattern for the license plate
license_plate_pattern = re.compile(r'^[A-Z]{1,3}-\d{1,3}-[A-Z]{1,3}$')
        ...
        for class_id, box_tensor in zip(result.boxes.cls, result.boxes.xyxy):
            ...
            if class_id == 2 or class_id == 3 or class_id == 5 or class_id == 7:
                ...
                try:
                    ...
                    # Check if the license plate matches the regex pattern
                    # if not license_plate_pattern.match(license_plate_text):
                    #     print(
                    #         "License plate does not match the pattern. Skipping entry.")
                    #     continue
